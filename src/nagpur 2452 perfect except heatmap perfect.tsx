...
const TILES_AVAILABLE_YEARS = new Set([2025]);
...
async function loadMainTiff(): Promise<TiffCache | null> {
  if (TIFF_CACHE) return TIFF_CACHE;
  if (TIFF_LOADING) return null;
  TIFF_LOADING = true;

  try {
    const tiff = await GeoTIFF.fromUrl("/cog/Nagpur_Weekly_2025_cog.tif");

    console.log("=== TIFF LOAD START ===");
...
// Each ramp maps a normalised value 0–1 → [R, G, B]
// t=0 = coldest/lowest, t=1 = hottest/highest
...
// LST: blue→cyan→yellow→orange→red  (cold→hot)
const LST_RAMP: [number,[number,number,number]][] = [
  [0.00, [30,  58, 200]],   // deep blue  ≈ cool
  [0.20, [59, 130, 246]],   // blue
  [0.40, [147,197,253]],    // light blue
  [0.55, [253,224, 71]],    // yellow     ≈ moderate
  [0.70, [251,146, 60]],    // orange
  [0.85, [239, 68, 68]],    // red        ≈ hot
  [1.00, [153, 27, 27]],    // deep red   ≈ extreme
];

// NDVI: red-brown→yellow→green
...
// Rain: light-blue→blue→dark-blue
...
// Water: very-light-blue→sky-blue→deep-blue
...
// LULC: discrete palette — class 0–8
...
function valueToRGBA(layer: LayerType, v: number): [number,number,number,number] {
  ...
  switch (layer) {
    case "lst":   rgb = sampleRamp(LST_RAMP,   t); break;
  ...
  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      ...
      if (n === 0) {
        ...
        // so sparse finite TIFF pixels are never lost in the blur pass.
        if (data[i4+3] > 0) {
...
function renderHeatmapToDataUrl(layer: LayerType, weekIndex: number): string | null {
  ...
  const [tiffMinX, tiffMinY, tiffMaxX, tiffMaxY] = tiffBbox;
  ...
  // The canvas image is pinned to NAGPUR_BBOX via L.imageOverlay.
  // sampleTiffValue() uses the TIFF's own bbox to map lat/lng → TIFF pixel.
  // To make tooltip and renderer 1:1 consistent, we render the canvas in
  // NAGPUR_BBOX coordinate space: each canvas pixel represents a fixed
  // lat/lng tile, and we look up which TIFF pixel covers that lat/lng.
  //
  // This guarantees: if sampleTiffValue(lat, lng) is finite,
  //   the canvas pixel at the same lat/lng screen position is also painted.
  //
  // Canvas resolution: match TIFF pixel density projected into NAGPUR_BBOX.
  const overlayMinLng = NAGPUR_BBOX.minLng; // 78.65
  const overlayMaxLng = NAGPUR_BBOX.maxLng; // 79.70
  const overlayMinLat = NAGPUR_BBOX.minLat; // 20.70
  const overlayMaxLat = NAGPUR_BBOX.maxLat; // 21.58

  // Compute how many TIFF pixels span the overlay extent in each axis.
  // This sets canvas size proportional to actual TIFF pixel density.
  const tiffLngSpan     = tiffMaxX - tiffMinX;
  const tiffLatSpan     = tiffMaxY - tiffMinY;
  const overlayLngSpan  = overlayMaxLng - overlayMinLng;
  const overlayLatSpan  = overlayMaxLat - overlayMinLat;

  // pixels-per-degree in the TIFF
  const tiffPpd_x = tw / tiffLngSpan;
  const tiffPpd_y = th / tiffLatSpan;

  // Canvas dimensions in OVERLAY coordinate space (may differ from tw/th)
  const W_raw = Math.round(overlayLngSpan * tiffPpd_x);
  const H_raw = Math.round(overlayLatSpan * tiffPpd_y);
  const UPSCALE = Math.min(2, Math.max(1, Math.floor(800 / Math.max(W_raw, H_raw))));
  const W = W_raw * UPSCALE;
  const H = H_raw * UPSCALE;

  console.log(
    `[Heatmap ${layer} wk${weekIndex}] Render alignment:` +
    ` TIFF bbox=[${tiffMinX.toFixed(4)},${tiffMinY.toFixed(4)},${tiffMaxX.toFixed(4)},${tiffMaxY.toFixed(4)}]` +
    ` OVERLAY bbox=[${overlayMinLng},${overlayMinLat},${overlayMaxLng},${overlayMaxLat}]` +
    ` TIFF dims=${tw}x${th}  Canvas=${W}x${H} (UPSCALE=${UPSCALE})`
  );

  const imgData = new Uint8ClampedArray(W * H * 4);

  // ── Pass 1: for every canvas pixel, compute its lat/lng and look up TIFF pixel ─
  // This is the key fix: canvas space = overlay space = tooltip space.
  // Every canvas pixel (cx, cy) maps to an exact lat/lng, which maps to an exact
  // TIFF pixel using the TIFF's own bbox. Tooltip uses the identical formula.
  let finitePixelCount = 0;

  for (let cy = 0; cy < H; cy++) {
    // Canvas y=0 is NORTH (maxLat), y=H-1 is SOUTH (minLat) — match TIFF row order
    const lat = overlayMaxLat - (cy / H) * overlayLatSpan;

    for (let cx = 0; cx < W; cx++) {
      const lng = overlayMinLng + (cx / W) * overlayLngSpan;

      // Map lat/lng → TIFF pixel using TIFF's own bbox — IDENTICAL to sampleTiffValue()
      const tx = Math.floor(((lng - tiffMinX) / tiffLngSpan) * tw);
      const ty = Math.floor(((tiffMaxY - lat) / tiffLatSpan) * th);

      if (tx < 0 || tx >= tw || ty < 0 || ty >= th) continue; // outside TIFF extent
  ...
  // ── Pass 2: edge feathering — neighbour-proximity only, never interior pixels ─
  // Only soften pixels adjacent to transparent neighbours (true boundary pixels).
  // Interior pixels are untouched. Hard floor = MIN_ALPHA ensures finite pixels
  // are never made invisible.
  const MIN_ALPHA = 80;

  for (let y = 0; y < H; y++) {
    for (let x = 0; x < W; x++) {
      const i4 = (y * W + x) * 4;
      if (imgData[i4+3] === 0) continue; // transparent — skip
      ...
      if (transparentN === 0) continue; // interior pixel — leave fully opaque

      // Feather: max 45% reduction, never below MIN_ALPHA
      const feather  = 1.0 - (transparentN / totalN) * 0.45;
      const newAlpha = Math.round(imgData[i4+3] * feather);
      imgData[i4+3]  = Math.max(MIN_ALPHA, newAlpha);
    }
  }

  // ── Pass 3: Gaussian box blur — sparse-region safe ─────────────────────────
  const blurred = boxBlur(imgData, W, H);

  // ── Pass 4: HARD ALPHA FLOOR — final guarantee ─────────────────────────────
  // Walk every canvas pixel that was finite in Pass 1.
  // If blur reduced it below ALPHA_FLOOR, restore pre-blur colour + floor alpha.
  const ALPHA_FLOOR = 80;
  let visibleCount    = 0;
  let floorCorrections = 0;

  for (let cy2 = 0; cy2 < H; cy2++) {
    for (let cx2 = 0; cx2 < W; cx2++) {
      const i4 = (cy2 * W + cx2) * 4;
      if (imgData[i4+3] === 0) continue; // was not a finite pixel in Pass 1
      if (blurred[i4+3] >= ALPHA_FLOOR) {
        visibleCount++;
      } else {
        blurred[i4]   = imgData[i4];
        blurred[i4+1] = imgData[i4+1];
        blurred[i4+2] = imgData[i4+2];
        blurred[i4+3] = ALPHA_FLOOR;
        floorCorrections++;
        visibleCount++;
      }
    }
  ...
  const visiblePct = finitePixelCount > 0
    ? ((visibleCount / finitePixelCount) * 100).toFixed(1) : "0.0";
  console.log(
    `[Heatmap ${layer} wk${weekIndex}] Visible rendered pixels: ${visibleCount} / ${finitePixelCount}` +
    ` (${visiblePct}%) | Floor corrections: ${floorCorrections}`
  );
  ...
  // Subtle CSS blur softens pixel blockiness
...
const CanvasHeatmapLayer = React.memo(({ activeLayer, weekIndex }: {
  activeLayer: LayerType; weekIndex: number;
}) => {
  const map        = useMap();
  ...
  const scheduleRender = useCallback(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      requestAnimationFrame(() => {
        ...
        } else if (TIFF_CACHE === null) {
          // TIFF not yet loaded — poll until it is
          const poll = () => {
            if (TIFF_CACHE) {
              const du = renderHeatmapToDataUrl(activeLayer, weekIndex);
              if (du) applyOverlay(du);
            } else {
              setTimeout(poll, 400);
            }
          };
  ...
  // Re-render on layer/week change
  useEffect(() => {
    scheduleRender();
  }, [scheduleRender]);

  // Re-render after map interactions stop (not during drag/pan)
  useEffect(() => {
    ...
    const onSettled   = () => { if (!isDragging.current) scheduleRender(); };
    ...
    <Card style={{ padding: "14px 16px" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <div style={{ marginTop: 4, padding: "5px 8px", background: "#f5f3ff", borderRadius: 7, border: "1px solid #ede9fe" }}>
          ...
          <span style={{ fontSize: 9, color: "#9ca3af" }}>Tiles: XYZ PNG · public/tiles/2025/&#123;z&#125;/&#123;x&#125;/&#123;y&#125;.png</span>
        </div>
      </div>
    </Card>
  );
...
function BandDiagnosticsPanel({ activeLayer, weekIndex }: { activeLayer: LayerType; weekIndex: number }) {
  ...
  // Classify current week: dead, saturated, or plausible
  ...
  // Per-row state classification for probe weeks
  ...
  // Show soil-specific unreliability banner
  ...
  return (
    <Card style={{ padding: "14px 16px", border: "1px solid #fcd34d", background: "#fffbeb" }}>
      ...
      {/* Soil unreliability banner — top priority warning */}
      {showSoilBanner && (
          ...
          {isDead && (
            <div style={{ marginTop:6, padding:"5px 8px", background:"#f1f5f9", border:"1px solid #94a3b8", borderRadius:6 }}>
              <span style={{ fontSize:9.5, color:"#1e3a5f", fontWeight:700 }}>
                ⬛ DEAD BAND: zero valid pixels this week. Band is fully masked or zero-filled in source TIFF.
              </span>
            </div>
          )}
          {isSaturated && !isDead && (
            <div style={{ marginTop:6, padding:"5px 8px", background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:6 }}>
              <span style={{ fontSize:9.5, color:"#991b1b", fontWeight:700 }}>
                ⚠️ SATURATED: all pixels ≈ same value ({curRaw.max.toFixed(4)}). GEE .min(1) clipping.
              </span>
            </div>
          )}
        </div>
      ...
      {/* GEE fix recommendation if saturated or dead */}
      ...
      <div style={{ marginTop:7, fontSize:8, color:"#b45309" }}>
        ...
        Band index = weekIndex × 6 + {BAND_OFFSETS[activeLayer]} ({LAYER_META[activeLayer].name}).
        {TIFF_CACHE && ` Valid pixels: ~${Math.round(((TIFF_CACHE.bands[0]?.length ?? 0) - (TIFF_CACHE.width * TIFF_CACHE.height - (rows[0]?.s?.count ?? 0))) / (TIFF_CACHE.width * TIFF_CACHE.height) * 100)}% NaN masked.`}
      </div>
    </Card>
  );
}
...
function AnalyticsContent({ stats, activeLayer, weekIndex, year, annualMeans }: {
  ...
  // fmtVal: format the TIFF-derived (already scale-corrected) value for display.
  // For soil: DETECTED_SCALE[soil] is already applied, value is in 0–1 range.
  // If the TIFF is saturated (all ≈ 1), showing "100.0%" is the honest truth.
  const fmtVal = (v: number): string => {
    switch (activeLayer) {
      case "lst":   return `${v.toFixed(1)}°C`;
      case "ndvi":  return v.toFixed(3);
      case "rain":  return `${v.toFixed(1)} mm`;
      case "soil":  return `${v.toFixed(4)} (${(v * 100).toFixed(1)}%)`;  // show raw fraction + %
        ...
        {activeLayer === "soil" && SOIL_DATA_UNRELIABLE && (
          <div style={{ marginBottom:8, padding:"6px 10px", background:"#fef2f2", border:"1px solid #fca5a5", borderRadius:7 }}>
            <span style={{ fontSize:9.5, color:"#7f1d1d", fontWeight:700 }}>
              Source TIFF temporally corrupt: some weeks saturated ≈1.0, others fully dead (0). Values shown are raw — no correction applied. See Band Diagnostics below.
            </span>
          </div>
        )}
...
function BasemapTiles({ mapType }: { mapType: MapType }) {
  ...
  if (mapType === "satellite") {
    return <TileLayer url="https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}" maxZoom={22} attribution="© Esri" />;
  }
  return (
    <>
...
//
// Value source: sampleTiffValue(lat, lng) — reads COG TIFF pixel from cache.
// If TIFF not loaded or pixel is nodata: shows "No Data" — no fake fallback.
//
// Performance:
//   • Single COG TIFF loaded once in module scope (prewarmTiffs called at mount)
//   • Hover sampling is throttled with requestAnimationFrame (~60fps cap)
//   • TIFF is NEVER rendered on canvas from here — sampling only
// ─────────────────────────────────────────────────────────────────────────────
...
// Inject global tile-smoothing + heatmap overlay CSS once
const GLOBAL_CSS = `
.nagpur-tiles img {
...
/**
 * getTooltipValue — reads value from COG TIFF only.
 * Returns null if TIFF not loaded or pixel is nodata (show "No Data" in tooltip).
 */
...
/** Returns true if the COG TIFF is loaded and ready */
...
const HoverTooltip = React.memo(({ weekIndex, year, annualMeans, activeLayer }: {
  ...
  // Pre-warm TIFFs as soon as this component mounts
  useEffect(() => { prewarmTiffs(); }, []);
  ...
  useMapEvents({
    mousemove(e) {
      ...
      // Throttle via rAF — skip if we already have a frame queued
  ...
  function buildTooltip(lat: number, lng: number) {
    ...
    // ── Source badge: shows "TIFF" if COG loaded, "Loading…" if not ──────────
    ...
    // ── Delta indicator: coloured arrow vs annual mean ──────────────────────
...
<div style="padding:7px 10px;background:#1e293b;display:flex;align-items:center;justify-content:space-between;gap:8px">
  ...
  <div style="background:rgba(249,115,22,0.18);border:1px solid rgba(249,115,22,0.3);border-radius:5px;padding:2px 7px;text-align:center;flex-shrink:0">
    ...
    if (activeLayer === "lst") {
      // Full 6-band multi-column tooltip — COG TIFF sampled values
...
<div style="font-family:system-ui,-apple-system,sans-serif;width:240px;background:#fff">
  ...
  ${footer}
    ...
    // ── Single-layer tooltip (non-LST layers) ───────────────────────────────
    ...
    if (raw !== null) {
      ...
      // TIFF not loaded or nodata pixel
  ...
  return (
    <div ref={dropRef} style={{ position: "absolute", bottom: 20, left: "50%", transform: "translateX(-50%)", zIndex: 700 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
        <button
          onClick={() => { if (weekIndex > 0) setWeekIndex(weekIndex - 1); else if (year > 2015) { setYear(year - 1); setWeekIndex(N_WEEKS - 1); } }}
        ...
        <button
          onClick={() => { if (weekIndex < N_WEEKS - 1) setWeekIndex(weekIndex + 1); else if (year < 2025) { setYear(year + 1); setWeekIndex(0); } }}
          style={{ background:"#fff",border:"1px solid #e5e7eb",borderRadius:12,width:36,height:36,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 2px 8px rgba(0,0,0,0.08)",flexShrink:0 }}
          onMouseEnter={e=>(e.currentTarget.style.background="#f8fafc")} onMouseLeave={e=>(e.currentTarget.style.background="#fff")}>
            ...
            <div style={{ display:"flex",flexWrap:"wrap",gap:6 }}>
              {YEARS.map(y => (
                <button key={y} onClick={() => setPickerYear(y)}
                  ...
                  {y}
                  {!TILES_AVAILABLE_YEARS.has(y) && (
                    <span style={{ position:"absolute",top:-4,right:-4,width:8,height:8,borderRadius:"50%",background:"#d1d5db",border:"1.5px solid #fff" }} title="Tiles not yet available" />
                  )}
                </button>
              ))}
            </div>
            {!TILES_AVAILABLE_YEARS.has(pickerYear) && (
              <div style={{ marginTop:8,padding:"5px 10px",background:"#fef9c3",border:"1px solid #fde047",borderRadius:8 }}>
                <span style={{ fontSize:10,color:"#713f12" }}>⚠️ Tiles not yet available for {pickerYear} — map will show 2025 tiles</span>
              </div>
            )}
          </div>
          <div style={{ marginBottom:14 }}>
      ...
      <div style={{ background:"rgba(255,255,255,0.97)",border:"1px solid #e5e7eb",borderRadius:999,padding:"6px 10px",boxShadow:"0 2px 12px rgba(0,0,0,0.08)",display:"flex",alignItems:"center",gap:4 }}>
        {YEARS.map(y => (
          <button key={y}
            ...
            {y}
            {!TILES_AVAILABLE_YEARS.has(y) && y !== year && (
              <span style={{ position:"absolute",top:1,right:2,width:5,height:5,borderRadius:"50%",background:"#d1d5db" }} />
            )}
          </button>
        ))}
      </div>
  ...
  return (
    <div style={{ position:"absolute",top:56,left:"50%",transform:"translateX(-50%)",zIndex:650,pointerEvents:"none" }}>
      <div style={{ background:"rgba(251,191,36,0.97)",border:"1px solid #f59e0b",borderRadius:999,padding:"4px 14px",boxShadow:"0 2px 8px rgba(0,0,0,0.10)",display:"flex",alignItems:"center",gap:6 }}>
        ...
        <span style={{ fontSize:11,fontWeight:700,color:"#92400e" }}>Tiles not yet available for {year} — displaying 2025 tiles</span>
      </div>
    </div>
  );
}
...
export default function Nagpur() {
  ...
  // Real TIFF-derived stats — recomputed whenever TIFF loads or selection changes
  ...
  // Poll until TIFF loads, then recompute stats
  useEffect(() => {
    let cancelled = false;
    const poll = () => {
      if (cancelled) return;
  ...
  // Recompute stats when layer or week changes
  useEffect(() => {
    setRealStats(getWeekStats(activeLayer, weekIndex));
  }, [activeLayer, weekIndex, tiffReady]);

  const tilesAvailable = TILES_AVAILABLE_YEARS.has(year);
  const tileYear       = tilesAvailable ? year : DEFAULT_YEAR;

  const setYear      = useCallback((y: number) => { setYearRaw(y); }, []);
    ...
    <div style={{ display:"flex", height:"100vh", width:"100%", fontFamily:"'Inter',system-ui,sans-serif", overflow:"hidden", background:"#f1f5f9" }}>
      ...
      <div style={{ flex:1, position:"relative", cursor:"crosshair" }}>
        ...
        >
          ...
          <CanvasHeatmapLayer activeLayer={activeLayer} weekIndex={weekIndex} />
          <NagpurBoundaryLayer />
          <HoverTooltip
            weekIndex={weekIndex}
            year={year}
        ...
        {!tilesAvailable && <TilesUnavailableBanner year={year} />}
      ...
      <aside style={{ width:272, flexShrink:0, display:"flex", flexDirection:"column", background:"#f8fafc", borderLeft:"1px solid #e5e7eb", overflowY:"auto", zIndex:10 }}>
        <div style={{ padding:"18px 16px 12px", borderBottom:"1px solid #e5e7eb", background:"#fff" }}>
          ...
          <p style={{ fontSize:11, color:"#9ca3af", marginTop:2 }}>
            ...
            {!tilesAvailable
              ? <span style={{ color:"#f59e0b", fontWeight:700 }}> · Tiles: 2025 proxy</span>
